 ...
 * Reads the active scenario from useSettingsStore so source-switch nodes
 ...
 * Cache is keyed by (nodeId, scenario) so switching scenarios doesn't serve
...
export function useDataInputColumns(
  ...
  // Scenario-aware cache key: "nodeId:scenario"
  const cacheKey = dataInput ? `${dataInput}:${activeScenario}` : ""
  ...
  useEffect(() => {
    ...
    previewNode(graph, dataInput, 1, activeScenario, { signal: controller.signal })
      .then((result) => {
        if (result.columns) {
          ...
          setColumnsCache(dataInput, result.columns, useNodeResultsStore.getState().graphVersion, activeScenario)
        }
      })
      .catch((e) => {
  ...
  }, [dataInput, allNodes, edges, submodels, preamble, activeScenario, setColumnsCache, cachedColumns, isCacheFresh, addToast])
